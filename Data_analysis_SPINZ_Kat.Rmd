...
pre <- clusters_df %>% filter(`Study timepoint`=="Baseline" | clusterid %in% c("ST307 1", "ST307 3")) %>%
  filter(clusterid !="ST101 1") %>%
  filter(clusterid != "Missing data" & clusterid !="NC") %>% 
  mutate(`Birth location`=fct_relevel(`Birth location`, "UTH born", "Outborn", "Unknown")) %>%
  mutate(onset_earlylate=fct_relevel(onset_earlylate, "Unknown", "Early", "Late")) %>%
  ggplot(aes(x=`Birth location`, fill=onset_earlylate)) + 
...
post <- clusters_df %>% 
  filter(`Study timepoint`!="Baseline" & !(clusterid %in% c("ST307 1", "ST307 3"))) %>%
  filter(clusterid != "Missing data" & clusterid !="NC") %>% 
  mutate(`Birth location`=fct_relevel(`Birth location`, "UTH born", "Outborn", "Unknown")) %>%
  mutate(onset_earlylate=fct_relevel(onset_earlylate, "Unknown", "Early", "Late")) %>%
  ggplot(aes(x=`Birth location`, fill=onset_earlylate)) + 
...
ggsave("pre_post_cluster_traits.pdf", width=6, height=7)


clusters_df %>% group_by(`Study timepoint`, onset_earlylate) %>% count()

clusters_df %>% 
  filter(clusterid != "Missing data" & clusterid !="NC") %>% 
  mutate(`Birth location`=fct_relevel(`Birth location`, "UTH born", "Outborn", "Unknown")) %>%
  mutate(onset_earlylate=fct_relevel(onset_earlylate, "Unknown", "Early", "Late")) %>%
  mutate(days=if_else(`Days to onset`>=7, 7, `Days to onset`)) %>%
  ggplot(aes(x=`Birth location`, y=days)) + 
  geom_violin() + 
  geom_jitter(aes(col=onset_earlylate), alpha=0.3) + 
  facet_wrap(~clusterid) +
  scale_color_manual(values=c("grey", "maroon", "navy")) + 
  theme_bw() + 
...
pre <- clusters_df %>% 
  ...
  mutate(onset_earlylate=fct_relevel(onset_earlylate, "Unknown", "Early", "Late")) %>%
  mutate(days=if_else(`Days to onset`>=7, 7, `Days to onset`)) %>%
  ggplot(aes(x=`Birth location`, y=`Days to onset`)) + 
  geom_violin() + 
  geom_jitter(aes(col=onset_earlylate), alpha=0.3) + 
  facet_wrap(~clusterid, scales="free_y") +
  scale_color_manual(values=c("grey", "maroon", "navy")) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle=60, hjust=1)) + ggtitle("Baseline")
...
post <- clusters_df %>% 
  ...
  scale_color_manual(values=c("grey", "maroon", "navy")) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle=60, hjust=1)) + ggtitle("Post-implementation")


pre / post + plot_layout(guides="collect")

ggsave("pre_post_cluster_days.pdf", width=6, height=7)
...
seg1 <- read_dna_seg_from_genbank("KL102.gbk")
seg2 <- read_dna_seg_from_genbank("KL102_disrupted.gbk")
...
comparisons <- read_comparison_from_blast("KL102_vs_disrupted.crunch.txt")

# colour the homology regions grey
comparisons$col <- "darkgray" 

pdf("KL102_comparison.pdf", width=9,height=3)
...
plot_gene_map(
  ...
  annotation_height = 2, 
  annotation_cex = 0.75,
  dna_seg_labels = list("a) Disrupted", "b) Wildtype")
)
