...
p_baseline <- clusters_df %>%
  ...
  scale_fill_manual(values = c("Rapid" = "#c51b8a", "Delayed" = "#08306b", "Unknown" = "grey"), name = "Onset category") +
  ...
  facet_wrap(~ ST_cluster, nrow = 4, ncol = 3, scales = "free_y") +
  theme_bw() +
...
#ggsave("figures/Supplementary_figure_3.png", width=8, height=8)
...
amr_mean_plot <- ggplot(amr_genes_long, aes(x = genes, y = mean, fill = `Study timepoint`)) +
  ...
  scale_x_discrete(labels = c("a_gly_acquired_r_mean" = "Aminoglycosides", "Col_acquired_r_mean" = "Colistin", "Fcyn_acquired_r_mean" = "Fosfomycin", "flq_acquired_r_mean" = "Fluoroquinolones", "Gly_acquired_r_mean" = "glycopeptides", "mls_acquired_r_mean" = "Macrolides", "Phe_acquired_r_mean" = "Phenicols", "Rif_acquired_r_mean" = "Rifampicin", "sul_acquired_r_mean" = "Sulfonamides", "tet_acquired_r_mean" = "Tetracyclines", "Tgc_acquired_r_mean" = "Tigecycline", "tmt_acquired_r_mean" = "Trimethoprim", "bla_acquired_r_mean" = "Penicillins", "bla_esbl_acquired_r_mean" = "3rd generation cephalosporins", "bla_carb_acquired_r_mean" = "Carbapenems")) +
  theme_classic() + # A clean theme
  theme(
...
#ggsave("figures/Figure_5.png", width=8, height=10)
...
#pdf("figures/Supplementary_figure_5.pdf", width=9,height=3)
...
st101_t_trimmed <- ggtree(st101_trimmed) 
...
# Plot
tree_hm <- subset_st101_t + geom_tiplab(aes(label = custom_label), nudge_x = 0.2, size = 3) + ggplot(dist_long, aes(x = ID2, y = ID1, fill = SNV_dist)) +
...
tree_hm
#ggsave("figures/Supplementary_figure_7.png", width=8, height=4)
...
all_res_binary <- read_csv("data/res_subset_seperated_binary.csv") %>% select(-all_of(c("-"))) %>% select_if(~ any(. != 0)) %>% select(-all_of(c("strB.v1", "strA.v1"))) %>%
  select_if(~ !all(is.na(.))) %>% 
